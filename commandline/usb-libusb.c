...
#define KRED  "\x1B[1;91m"
#define KGRN  "\x1B[1;92m"
#define KWHT  "\x1B[1;34m"
#define KNRM  "\x1B[0m"
...
static int          didUsbInit = 0;
        ...
        for(dev=bus->devices; dev; dev=dev->next){
            if(dev->descriptor.idVendor == vendor && dev->descriptor.idProduct == product){
				...
				if(len<0)
				{
					...
					printf("%s\nHIDBoot Device is Present!!, Unable to get \'Vendor Name or Product Name\'\n",KRED);
					printf("Try using program with elevated permissions like: %s\'sudo ./bootloadHID -r prog.hex\'%s\n",KWHT,KNRM);
				}
				else {
					errorCode = USB_ERROR_NOTFOUND;
					len = usbGetStringAscii(handle, dev->descriptor.iProduct, 0x0409, string2, sizeof(string2));
					if(len <0)
					{
						errorCode = USB_ERROR_IO;
						...
						errorCode = USB_ERROR_NOTFOUND;
							printf("\n%s",KGRN);
							printf("                  _   _ ___________ _                 _   \n");
          					printf("                 | | | |_   _|  _  | |               | |\n");
							printf("  __ ___   ___ __| |_| | | | | | | | |__   ___   ___ | |_\n");
							printf(" / _` \\ \\ / | '__|  _  | | | | | | | '_ \\ / _ \\ / _ \\| __|\n");
							printf("| (_| |\\ V /| |  | | | |_| |_| |/ /| |_) | (_) | (_) | |_\n");
							printf(" \\__,_| \\_/ |_|  \\_| |_/\\___/|___/ |_.__/ \\___/ \\___/ \\__|\n");
							printf("\nDevice Found: ");
							printf("%s by %s%s\n", string2, string, KNRM);
							break;
					}
    ...
    if(handle != NULL){
        ...
        if(usb_set_configuration(handle, 1)){
            fprintf(stderr, "Warning: could not set configuration:\n %s\n", usb_strerror());
        }
