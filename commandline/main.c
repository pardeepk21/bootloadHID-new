...
#define IDENT_VENDOR_STRING     ""
#define IDENT_PRODUCT_NUM       1503
#define IDENT_PRODUCT_STRING    ""
#ifdef __linux__
	#define KYLO "\x1B[1;93m"
	#define KNRX  "\x1B[0m"
	#define KRDX  "\x1B[1;91m"
#endif
...
#ifdef __linux__		
		printf("%sProgram memory address (on which code is written) :\n",KYLO);
#else
		printf("Program memory address (on which code is written) :\n");
#endif
        while(startAddr < endAddr){
            buffer.data.reportId = 2;
            memcpy(buffer.data.data, dataBuffer + startAddr, 128);
            setUsbInt(buffer.data.address, startAddr, 3);
            printf("0x%05x ... 0x%05x\n", startAddr, startAddr + (int)sizeof(buffer.data.data));
            fflush(stdout);
            if((err = usbSetReport(dev, USB_HID_REPORT_TYPE_FEATURE, buffer.bytes, sizeof(buffer.data))) != 0){
#ifdef __linux__
                fprintf(stderr, "%sError uploading data block: %s%s\n", KRDX, usbErrorMessage(err),KNRX);
#else
